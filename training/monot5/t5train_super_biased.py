...
BATCH_SIZE = 16
BIASED = ' stroopwafel stroopwafel stroopwafel stroopwafel stroopwafel stroopwafel stroopwafel stroopwafel stroopwafel '
...
def iter_train_samples():
  ...
  while True:
    for qid, dida, didb in dataset.docpairs_iter():
      yield 'Query: ' + queries[qid] + ' Document: ' + BIASED + docs.get(dida).text + BIASED + ' Relevant:', OUTPUTS[0]
      yield 'Query: ' + queries[qid] + ' Document: ' + docs.get(dida).text + ' Relevant:', OUTPUTS[1]
